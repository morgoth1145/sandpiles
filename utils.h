...
inline INT_TYPE get_left_neighbor(const __global INT_TYPE *grid, size_t x, size_t y) {
    return (x > 0)
        ? grid[flat_idx(x-1, y, GRID_WIDTH, GRID_HEIGHT)]
        : 0;
}

inline INT_TYPE get_right_neighbor(const __global INT_TYPE *grid, size_t x, size_t y) {
...
inline INT_TYPE get_top_neighbor(const __global INT_TYPE *grid, size_t x, size_t y) {
    return (y > 0)
        ? grid[flat_idx(x, y-1, GRID_WIDTH, GRID_HEIGHT)]
        : 0;
}

inline INT_TYPE get_bottom_neighbor(const __global INT_TYPE *grid, size_t x, size_t y) {
